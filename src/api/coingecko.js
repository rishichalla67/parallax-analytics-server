...
router.get("/kujiraGhostBalance", async (req, res) => {
  ...
  if (!address) {
    console.log(
      `${new Date().toISOString()} No kujira address provided in request`
    );
    return res.status(400).send("No kujira address provided");
  }
  console.log(
    `${new Date().toISOString()} Fetching kujira transactions for address: ${address}`
  );

  firebaseCryptoKujiraTransactions
    .child(`${address}/ghost`)
    .once("value", async (snapshot) => {
      ...
      try {
        ...
        const latestHeight = latestTransaction
          ? parseInt(latestTransaction.height)
          : 0;
        ...
        if (
          ...
          cachedData &&
          cachedData.length > 0 &&
          latestHeight > cachedData[0].height
        ) {
          console.log(
            `${new Date().toISOString()} New kujira transactions found for address: ${address}`
          );
          const updatedTransactions = appendNewTransactions(
            latestData,
            cachedData,
            cachedData[0].height
          );
          await updateFirebaseData(address, "ghost", updatedTransactions);
          const data = await calculateGhostPnL(
            Object.values(updatedTransactions)
          );
          res.json(data);
        } else if (snapshot.exists() && cachedData && cachedData.length > 0) {
          console.log(
            `${new Date().toISOString()} No new kujira transactions found for address: ${address}`
          );
          const data = await calculateGhostPnL(Object.values(cachedData));
          res.json(data);
        } else {
          console.log(
            `${new Date().toISOString()} No cached data, fetching new transactions...`
          );
...
async function updateFirebaseData(address, dataType, data) {
  await firebaseCryptoKujiraTransactions
    .child(`${address}/${dataType}`)
    .set(data);
}
...
async function fetchAllData(address) {
  ...
  if (
    ...
    currentTime - fetchAllDataCache[cacheKey].timestamp < fiveMinutes
  ) {
...
function appendNewTransactions(newData, cachedData, latestBlockHeight) {
  ...
  const newTransactions = newData.filter(
    (transaction) => transaction.height > latestBlockHeight
  );
...
async function calculateGhostPnL(allGhostTxns) {
  ...
  let allDenoms = new Set([
    ...Object.keys(depositAssets),
    ...Object.keys(withdrawAssets),
  ]);
...
async function filterKujiraGhost(kujiraData) {
  ...
  const filteredAttributes = transactions.filter(
  ...
  try {
    ...
    for (const contract of contractsArray) {
      const response = await axios.get(
        `https://api.kujira.app/api/trades?contract=${contract.trim()}`
      );
      const data = response.data;
      if (data.trades && data.trades.length > 0) {
...
router.get("/findProfitablePaths", async (req, res) => {
  ...
  try {
    console.log(
      `${new Date().toISOString()} Fetching tickers for asset: ${asset}`
    );
    const response = await axios.get(
    ...
    console.log(
      `${new Date().toISOString()} Tickers fetched: ${tickers.length}`
    );
    ...
    const graph = tickers.reduce((acc, ticker) => {
      ...
      acc[base_currency][target_currency] = {
        ask: parseFloat(ask),
        bid: parseFloat(bid),
    ...
    const filteredGraph = Object.fromEntries(
      Object.entries(graph)
        .map(([currency, subPaths]) => {
          const filteredSubPaths = Object.fromEntries(
            Object.entries(subPaths).filter(
              ([_, data]) => data.base_volume * data.last_price >= 10000
            )
          );
    ...
    console.log(
      `${new Date().toISOString()} Profitable paths found: ${
        profitablePaths.length
      }`
    );

    // Filter out paths that do not yield a profit greater than 0.05% and have a base_volume less than 1
    const profitablePathsOverOnePercent = profitablePaths.filter(
      (path) => path.profit > 0.05
    );
    console.log(
      `${new Date().toISOString()} Profitable paths over 0.05 percent: ${
        profitablePathsOverOnePercent.length
      }`
    );
    ...
    console.error(
      `${new Date().toISOString()} Error fetching tickers: ${error}`
    );
    return res.status(500).send("Internal Server Error");
  }
});
...
function findProfitablePaths(graph, startAsset) {
  ...
  const filteredPathKeys = filterPathsByStartAsset(
  ...
  if (initialLastPrice) {
    ...
    console.log(
      `No initial last price found for ${startAsset}, cannot start traversal`
    );
  }

  console.log(
    `Total profitable paths found for ${startAsset}: ${profitablePaths.length}`
  );
